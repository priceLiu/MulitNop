--add by hz full page
CREATE PROCEDURE [dbo].[ProductLoadAllPagedHzVendor]
(
	@CategoryIds		nvarchar(MAX) = null,	--a list of category IDs (comma-separated list). e.g. 1,2,3
	@ManufacturerId		int = 0,
	@VendorId			int = 0,--add by hz
	@ProductTagId		int = 0,
...
BEGIN
	...
	BEGIN
		SET @sql = @sql + '
		LEFT JOIN Product_Vendor_Mapping psm with (NOLOCK)
	...
	BEGIN
		SET @sql = @sql + '
		AND psm.VendorId = ' + CAST(@VendorId AS nvarchar(max))
		
		IF @FeaturedProducts IS NOT NULL
		BEGIN
			SET @sql = @sql + '
		AND psm.IsFeaturedProduct = ' + CAST(@FeaturedProducts AS nvarchar(max))
		END
	END
	--end by hz
	...
	BEGIN
		...
		BEGIN
			IF LEN(@sql_orderby) > 0 SET @sql_orderby = @sql_orderby + ', '
			SET @sql_orderby = @sql_orderby + ' psm.DisplayOrder ASC'
		END
		--end by hz
		--name
		IF LEN(@sql_orderby) > 0 SET @sql_orderby = @sql_orderby + ', '
